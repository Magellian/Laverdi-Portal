...
export default function DashboardSidebar({ email }: { email: string | null | undefined }) {
  ...
  const sidebar = (
    <aside className="flex h-full w-[280px] flex-col bg-zinc-900 border-r border-zinc-800">
      <div className="flex h-16 items-center px-6 border-b border-zinc-800 flex-shrink-0">
        <span className="text-xl font-bold text-white tracking-tight">LaVerdi</span>
      </div>
      ...
      <div className="border-t border-zinc-800 px-4 py-4 flex-shrink-0">
        ...
        <button
          ...
          className="w-full rounded-lg bg-zinc-800 px-3 py-2 text-sm text-zinc-300 hover:text-white hover:bg-zinc-700 transition-colors text-left"
        >
          Sign Out
        </button>
      </div>
