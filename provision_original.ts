...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const cloudInitScript = [
      ...
      'curl -sf -X POST ' + PORTAL_URL + '/api/webhooks/instance-ready -H "Content-Type: application/json" -d \'{"instanceId":"' + instanceId + '","userId":"' + userId + '","instanceIp":"\'"$INSTANCE_IP"\'","instancePort":9000,"token":"' + WEBHOOK_TOKEN + '"}\'  || echo "Webhook failed"',
