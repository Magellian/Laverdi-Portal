-- Migration: Add tier-based model system
-- Date: 2026-04-22
-- Description: Add tier, model_id, and model tier mapping to Laverdi portal
...
-- 2. Create model_tier_map table (tier → model mapping)
...
-- 3. Populate initial tier mappings
...
-- 4. Update existing users to have tier='free' and correct model
UPDATE users SET
  tier = 'free',
...
-- 6. Add RLS policies for model_tier_map (public read, admin write)
ALTER TABLE model_tier_map ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Enable read access for all users" ON model_tier_map
  FOR SELECT USING (true);

CREATE POLICY "Enable write access for service role" ON model_tier_map
  FOR INSERT, UPDATE, DELETE
  USING (auth.role() = 'service_role');

-- 7. Create instances table if not exists (for tracking OpenClaw containers)
...
-- 8. Set up RLS for instances (users can only see their own)
