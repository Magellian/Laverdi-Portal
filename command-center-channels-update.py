# This is the improved configure_channels() function to replace the existing one
# It includes full implementations for Telegram, Discord, Slack, and Signal
    ...
    try:
        ...
        for channel_name, config in channels.items():
            ...
            # ─── SIGNAL ───────────────────────────────────────────────────────
                ...
                try:
                    # Store Signal configuration (no API validation for Signal, as it requires daemon)
                    supabase.table('channels').upsert({
                        'user_id': user_id,
                        'channel_name': 'signal',
                        'enabled': config.get('enabled', True),
